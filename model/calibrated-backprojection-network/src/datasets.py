...
def load_depth_triplet(path):
    '''
    Load depth maps from depth triplet

    Arg(s):
        path : str
            path to depth triplet
    Return:
        numpy[float32] : depth at time t (C x H x W)
        numpy[float32] : depth at time t-1 (C x H x W)
        numpy[float32] : depth at time t+1 (C x H x W)
    '''

    # Load image triplet and split into images at t-1, t, t+1
    depths = load_depth(path)

    # Split along width
    depth1, depth0, depth2 = np.split(depths, indices_or_sections=3, axis=-1)

    return depth1, depth0, depth2

def load_intrinsics_triplet(path):
    '''
    Load intrinsics from intrinsics triplet

    Arg(s):
        path : str
            path to intrinsics triplet
    Return:
        numpy[float32] : intrinsics at time t (C x H x W)
        numpy[float32] : intrinsics at time t-1 (C x H x W)
        numpy[float32] : intrinsics at time t+1 (C x H x W)
    '''

    # Load image triplet and split into images at t-1, t, t+1
    intrinsics = np.load(path).astype(np.float32)

    # Split along width
    intrinsics1, intrinsics0, intrinsics2 = np.split(intrinsics, indices_or_sections=3, axis=-1)

    return intrinsics1, intrinsics0, intrinsics2
...
class KBNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load depth
        depth1, sparse_depth0, depth2 = load_depth_triplet(
            self.sparse_depth_paths[index])

        # Load camera intrinsics
        intrinsics1, intrinsics, intrinsics2 = load_intrinsics_triplet(
...
class PoseNetTrainingDataset(torch.utils.data.Dataset):
    '''
    Dataset for fetching:
        (1) image at time t-1, t, and t+1
        (2) depth
    ...
    def __getitem__(self, index):
        ...
        # Load depth
        depth1, depth0, depth2 = load_depth_triplet(
            self.depth_paths[index])

        # Load camera intrinsics
        intrinsics1, intrinsics0, intrinsics2 = load_intrinsics_triplet(
            self.intrinsics_paths[index])

        # Crop image, depth and adjust intrinsics
        # if self.do_random_crop:
        #     [image0, image1, image2, depth0], intrinsics = random_crop(
        #         inputs=[image0, image1, image2, depth0],
        #         shape=self.shape,
        #         intrinsics=intrinsics)
                # crop_type=self.random_crop_type)

        # Convert to float32
        image0, image1, image2, depth0, depth1, depth2, intrinsics0, intrinsics1, intrinsics2 = [
            T.astype(np.float32)
            for T in [image0, image1, image2, depth0, depth1, depth2, intrinsics0, intrinsics1, intrinsics2]
        ]

        return image0, image1, image2, depth0, depth1, depth2, intrinsics0, intrinsics1, intrinsics2
