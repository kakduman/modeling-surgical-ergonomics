...
import generic_unet_RGB_datasets as datasets

sys.path.insert(0, 'src')
import data_utils
from eval_utils import validateUnetRGB
from log_utils import log, save_RGB_prediction_img, save_unet_RGB_prediction_img
...
def train(train_image_path,
    ...
    log('Training input path:', log_path)
    ...
    train_dataloader = torch.utils.data.DataLoader(
        datasets.GenericUNetRGBTrainingDataset(
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        for train_image, train_ground_truth in train_dataloader:
            ...
            [train_image], [train_ground_truth] = train_transforms.transform(
                images_arr=[train_image],
            ...
            output_logits = model.forward(train_image)
...
def run(image_path,
    ...
    if do_visualize_predictions:
        ...
        visual_paths = [
            os.path.join(checkpoint_path, image_path.split('/')[-1].replace('.txt', ''))
        ]
    ...
    dataloader = torch.utils.data.DataLoader(
        datasets.GenericUNetRGBInferenceDataset(
    ...
    with torch.no_grad():
        ...
        if ground_truth_path is not None:
            best_results = validateUnetRGB(
                model=model,
                dataloader=dataloader,
                transforms=transforms,
                save_prediction_img=save_unet_RGB_prediction_img,
