...
class GenericUNetRGBModel(object):
    '''
    ...
    def forward(self, input_image):
        ...
        Returns:
            list[tensor] : segmentation in a list
        '''

        self.input_image = input_image

        # Forward original tensor through segmentation encoder
        latent_segmentation, skips_segmentation = self.encoder_segmentation(self.input_image)
    ...
    def log_summary(self,
                    ...
                    n_display=1):
        '''
        Log the input image, output logits, ground truth, error, and scalars to tensorboard
        ...
        with torch.no_grad():
            ...
            if len(input_image.shape) == 5:
                _, n_chunk, _, o_height, o_width = input_image.shape
            else:
                _, n_chunk, o_height, o_width = input_image.shape

            if n_chunk > 1:
                # Shape: N x C x D x H x W
                if len(input_image.shape) == 5:
                    input_scan = torch.unsqueeze(
                        input_image[0:n_display, n_chunk // 2, 0, :, :], dim=1)
                # Shape: N x C x H x W or N x D x H x W or N x (C x D) x H x W
                else:
                    input_scan = torch.unsqueeze(
                        input_image[0:n_display, n_chunk // 2, :, :], dim=1)
            else:
                # Shape: N x 1 x D x H x W
                if len(input_image.shape) == 5:
                    input_scan = input_image[0:n_display, :, 0, :, :]
                # Shape: N x 1 x H x W
                else:
                    input_scan = input_image[0:n_display, ...]

            # Logging segmentation outputs to summary

            # Make output_segmentation into multiclass segmentation
            n_height, n_width = output_logits.shape[-2:]

            output_logits = output_logits[0:n_display]

            output_softmax = torch.softmax(output_logits.to(torch.float32), dim=1)
