...
class GenericUNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for idx, scan_paths in enumerate(self.multimodal_scan_paths):
            ...
            # for gray, use 'L' and then reshape
            # if want RGB, use 'RGB' and remove the reshape
            image = Image.open(scan_path).convert('L')
            scan = np.asarray(image, np.float32)
            scan = scan.reshape((scan.shape[0], scan.shape[1], 1))
            # scan = np.load(scan_path).astype(np.float32)

            # MODIFIED - stack 3 on top of each other
            scan = np.concatenate((scan, scan, scan), axis=2)
        ...
        # for gray, use 'L' and then reshape
        # if want RGB, use 'RGB' and remove the reshape
        # ground_truth = np.asarray(Image.open(self.ground_truth_paths[index]).convert('RGB'), np.float32)
