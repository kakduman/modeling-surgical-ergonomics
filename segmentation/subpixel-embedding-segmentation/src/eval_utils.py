...
def average_flip(model, output, image, flip_type):
    ...
    if is_hflip:
        ...
        image_hflip = torch.from_numpy(image_hflip)
        # get the prediction for the flipped image:
        output_hflip = model.forward(image_hflip)[-1].cpu().data.numpy()
    ...
    if is_vflip:
        ...
        # convert the image into torch tensor:
        image_vflip = torch.from_numpy(image_vflip)
        # get the prediction for the flipped image:
        output_vflip = model.forward(image_vflip)[-1].cpu().data.numpy()
        # flip the prediction back:
    ...
    if is_both_flip:
        ...
        # convert the image into torch tensor:
        image_bothflip = torch.from_numpy(image_bothflip)
        # get the prediction for the flipped image:
        output_bothflip = model.forward(image_bothflip[-1].cpu().data.numpy())
        # flip the prediction back:
...
def validateRGB(model,
    ...
    for idx, scan in enumerate(dataloader):
        ...
        # Save scan, segmentation, and ground truth chunk for tensorboard
        if summary_writer is not None and (np.random.rand() < 0.10 or idx == n_samples - 1) and ground_truths is not None:
...
def validateUnetRGB(model,
    ...
    for idx, scan in enumerate(dataloader):
        ...
        # convert into multiclass segmentation map
        output_softmax = torch.softmax(output_logits, dim=1)
        output_segmentation = torch.argmax(output_softmax, dim=1)
