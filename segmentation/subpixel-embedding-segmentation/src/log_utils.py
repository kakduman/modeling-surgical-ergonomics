...
def save_MRI_prediction_img(chunk,
    ...
    for modality_id in range(len(visual_paths)):
        ...
        if len(ground_truth_2d) != 0:
            visual_outputs.append((image_gt_color_path, chunk_mod_gt, 'viridis'))
            visual_outputs.append((image_gt_gray_path, ground_truth_2d_gray, 'gray'))
...
def save_RGB_prediction_img(chunk,
    ...
    for modality_id in range(len(visual_paths)):
        ...
        if len(ground_truth_2d) != 0:
            ...
            combined = np.concatenate([chunk, chunk_pred], axis=1)
        ...
        if len(ground_truth_2d) != 0:
            visual_outputs.append((image_gt_color_path, chunk_gt))
            visual_outputs.append((image_gt_gray_path, ground_truth_2d_gray))

        visual_outputs.append((combined_color_path, combined))
...
def save_unet_RGB_prediction_img(chunk,
                        idx,
                        output_segmentation,
                        color_to_class_map,
                        visual_paths):
    '''
    For each 2D prediction, save original scan (BW) and predictions (color)
    ...
    Arg(s):
        ...
        output_segmentation : numpy[float32]
            H x W segmentation prediction
        color_to_class_map: dict
            tuple -> label dictionary
        visual_paths : list[str]
            1 element for each modality for path to save each modality img
    '''
    ...
    for modality_id in range(len(visual_paths)):
        ...
        # get the colors corresponding to labels, skip background
        for color, class_label in color_to_class_map.items():
            if class_label != 0:
                chunk_pred[np.where(output_segmentation == class_label)] = color

        # now create a map that makes the background all 0s
        chunk_pred_map = np.copy(chunk_pred)
        chunk_pred_map[np.where(output_segmentation == 0)] = (0, 0, 0)

        # Add 1's in C dimension to convert from RGB -> RGBA
        alphas = np.ones((chunk.shape[0], chunk.shape[1], 1))
        chunk = np.concatenate([chunk, alphas], axis=-1)

        # also add the alphas to the predictions
        chunk_pred = np.concatenate([chunk_pred, alphas], axis=-1)
        chunk_pred_map = np.concatenate([chunk_pred_map, alphas], axis=-1)

        combined = np.concatenate([chunk, chunk_pred, chunk_pred_map], axis=1)
        ...
        image_pred_color_path = os.path.join(
            ...
            'pred_color_overlay',
            'pred_color_patient%d.png' % (idx))
        image_pred_map_path = os.path.join(
            visual_paths[modality_id],
            'pred_map',
            'pred_map_patient%d.png' % (idx))
        combined_color_path = os.path.join(
            visual_paths[modality_id],
            'combined_color',
            'combined_color_patient%d.png' % (idx))
        ...
        visual_outputs = [
            (image_scan_color_path, chunk),
            (image_pred_color_path, chunk_pred),
            (image_pred_map_path, chunk_pred_map),
